...
module.exports.parse = function (app_name, task_obj) {
    ...
    var fn = core_loader.function_load(app_name,
        core_loader.config.sys['app_tasks_dir'],
        task_obj['fn']
    );
    // 使用 later 产生定时任务
    return function () {
        return later.setInterval(fn, schedParse(task_obj));
    };
};

// 根据app名称和task名称获取对应的函数
module.exports.getTask = function (app_name, task_name) {
    var task_str = config_load(app_name, 'tasks')[task_name];
    return core_loader.function_load(app_name,
        core_loader.config.sys['app_tasks_dir'],
        task_str
    );
