...
var global_route_namespace = core_loader.config['sys']['global_route_namespace'];

module.exports.parse = function (app_name, app_namespace, route_str, callback_str) {
    ...
    var route_arr = route_str.split(' '),
        url_prefix = [''];
    if (global_route_namespace) {
        url_prefix.push(global_route_namespace);
    }
    if (app_namespace) {
