...
fn.incrBy = function (field, num, callback) {
...
// 高并发下面 Mysql 中的数据有一定可能性的错误率，需要注意
fn.incrFloatBy = function (field, num, callback) {
    if (this._options['mysql_only']) {
...
var _getDirtyFields = function () {
    ...
    for (field in this._fields) if (this._fields.hasOwnProperty(field)) {
        ...
        if (field_obj['dirty']) {
            var parser = type_parse[field_obj['type']];
            result[field] = parser ?
                parser.dbFormat(parser['value']) : parser['value'];
        }
    }
    return result;
};
...
// 写入数据库新建/修改队列
fn.saveDB = function (callback, fields) {
    if (this._options['redis_only']) {
        return callback && callback()
    }
    var sql_params = {
        table: this._options['table_name'],
        content: fields || _getDirtyFields.call(this),
    ...
    mysql_queue.push(sql_params, callback);
};

// 直接更新 redis
fn.saveRedis = function (callback, fields) {
    if (this._options['mysql_only']) {
        return callback && callback()
    }
    var self = this;
    db_pool.redis.hmset(this['redis_key'], fields || _getDirtyFields.call(this),
        function (err) {
            if (err) throw Error(err);
...
fn.save = function (callback) {
    ...
    this.saveDB(function () {
        self.saveRedis(callback, fields);
    }, fields);
};

// 将数据从 redis 中删除
fn.destroyRedis = function (callback) {
    if (this._options['mysql_only']) {
        return callback && callback()
    }
...
// 添加 Mysql 数据删除队列
fn.destroyDB = function (callback) {
    if (this._options['redis_only']) {
        return callback && callback()
    }
    mysql_queue.push(
