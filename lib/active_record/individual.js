...
fn.init = function (model, params, is_new) {
    ...
    this._fields = JSON.parse(JSON.stringify(model._fields));
    // 遍历所有字段，添加当前值
    // 新数据 dirty 默认为 true
    ...
    for (func in model.meta) if (model.meta.hasOwnProperty(func)) {
        this[func] = model.meta[func];
    }
    this._is_new = is_new;
    // 初始化 redis_id
    if (!this._options['mysql_only']) {
        this['redis_key'] = this._options['redis_prefix'] + this['id'];
    }
};
